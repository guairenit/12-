<template>
  ...
  <script>
  import { getdisscuss, deldisscuss } from "@/api/userDisscuss";
  export default {
    name: "APP",
    data() {
      return {
    ...
    methods: {
      async getList() {
        const res = await getdisscuss({ uid: 1, currentPage: this.currentPage });
